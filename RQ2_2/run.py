...
def run(source, targets, reps, measure, perc=0.4):
    ...
    for target in targets:
        ...
        for _ in xrange(reps):
            ...
            source_train_indexes = indexes[:int(len(indexes) * perc)]
            source_subset = train_content.ix[source_train_indexes]
            source_train_indep = source_subset[ctrain_indep]
            source_train_dep = source_subset[ctrain_dep]

            # Train a model using source data
            source_model = DecisionTreeRegressor()
            source_model.fit(source_train_indep, source_train_dep)

            # Get indexes to split to train and testing data
            indexes = range(len(target_content))
            random.shuffle(indexes)

            # Get training and testing indexes to split into train pool and test pool
            all_train_indexes = indexes[:int(len(target_content) * 0.4)]
            target_test_indexes = indexes[int(len(target_content) * 0.4):]
            target_train_indexes = all_train_indexes[:int(len(all_train_indexes) * perc)]
            assert(len(target_train_indexes) <= len(all_train_indexes)), "Something is wrong"

...
def rdivDemo(data, isLatex=True, globalMinMax=False, high=100, low=0):
  if isLatex:
...
def sk_ranks(data, isLatex=True, globalMinMax=False, high=100, low=0):
    def z(x):
        return int(100 * (x - lo) / (hi - lo + 0.00001))
    data = map(lambda lst: Num(lst[0], lst[1:]),
                data)
    ...
    if globalMinMax:
        ...
        lo, hi = all[0], all[-1]
    last = None
    return [x for _, __, x in sorted(ranks)]
